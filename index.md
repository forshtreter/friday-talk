...
## BE<span style="color:#999">(</span>M<span style="color:#999">|</span>ViS<span style="color:#999">)</span>
{:.shout}
...
## View и State
* ...+ Надёжность
* ...&minus; Трудно вносить изменения
...
~~~
Button_size($size) {
    // как есть, только без border-radius
}
...
* Иметь контроль над порядком применения стилей
    * ...Все стили конкретного блока применяются в одном месте
